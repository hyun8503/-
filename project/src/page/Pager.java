...
public class Pager {
	...
	public static final int BLOCK_SCALE=10; //페이지 블록 개수 (블록 개수가 뭐지??)
	...
	private int blockStart; //페이지 블록의 시작페이지 번호
	private int blockEnd; //페이지 블록의 끝페이지 번호
	
	//Pager(레코드 개수, 보여줄 페이지 번호)
	public Pager(int count, int curPage) {
		curBlock=1;//페이지 블록을 1로 초기화
		this.curPage=curPage;
		setTotPage(count); //전체 페이지 개수 계산
		setPageRange();//#{start},#{end} 값 계산
		setTotBlock(); //페이지 블록의 개수 계산
		setBlockRange();//페이지 블록의 범위 설정
	}
